/*
*This Code plays a tune consisting of 6 predetermined notes for a predetermined time.
*After the last note is played, the first note plays again.
*This tune is broadcast over AM radio bandwidths.
*/
/*
...
static inline void transmitBeep(uint16_t pitch, uint16_t duration) {
  ...
  do {
    _delay_ms(1);
    PORTD = 0b00000000;                            /* delay for pitch cycles */
    duration--;
  } while (duration > 0);
